...
namespace BattleSystemECS.Tests
{
    ...
    ///  20. shrine_towers.json file exists and contains 3 definitions
    /// </summary>
    public class TowerShrineSystemTests
    {
        ...
        [Fact]
        public void ShrineTowersJson_ExistsWithThreeDefinitions()
        {
            // The JSON config file should exist and contain 3 shrine definitions
            // (the v1 template set: Gold, Mana, Battle).
            var path = System.IO.Path.Combine("..", "..", "..", "..", "Data", "Configs", "shrine_towers.json");
            Assert.True(System.IO.File.Exists(path),
                $"Expected shrine_towers.json at {path}");
            var content = System.IO.File.ReadAllText(path);
            Assert.Contains("Gold Shrine", content);
            Assert.Contains("Mana Shrine", content);
            Assert.Contains("Battle Shrine", content);
        }
