...
namespace BattleSystemECS.Tests
{
    ...
    ///  18. beacon_towers.json file exists and contains 3 definitions
    ///  19. tower_placement.json contains "12" entry in maxPerTypeByType
    ///  20. Two-beacon cluster: each in range of all 3 targets
    /// </summary>
    public class TowerBeaconSystemTests
    {
        ...
        private static (ComponentStore store, MockRenderer renderer, TowerPlacementSystem tps) Env()
        {
            ...
            return (store, new MockRenderer(), new TowerPlacementSystem(store, new MockRenderer()));
        }
        ...
        [Fact]
        public void BeaconTowersJson_FileExists_ContainsThreeDefinitions()
        {
            // Round 177 Direction 2 — designer-facing JSON config must exist with 3 definitions
            var path = System.IO.Path.Combine("..", "..", "..", "..", "Data", "Configs", "beacon_towers.json");
            Assert.True(File.Exists(path), $"beacon_towers.json not found at {path}");
            string json = File.ReadAllText(path);
            Assert.Contains("Outpost Beacon", json);
            Assert.Contains("Battle Beacon", json);
            Assert.Contains("Command Beacon", json);
        }

        [Fact]
        public void TowerPlacementJson_ContainsMaxPerType12()
        {
            // Round 177 Direction 2 — placement cap for Beacon (TowerType.Beacon = 12) must be set
            var path = System.IO.Path.Combine("..", "..", "..", "..", "Data", "Configs", "tower_placement.json");
            Assert.True(File.Exists(path), $"tower_placement.json not found at {path}");
            string json = File.ReadAllText(path);
            Assert.Contains("\"12\"", json); // TowerType.Beacon = 12 entry
        }
    }
}
