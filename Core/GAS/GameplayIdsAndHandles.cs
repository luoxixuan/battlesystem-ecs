...
namespace BattleSystemECS.Core.GAS
{
    ...
    /// <summary>墓碑查询：区分从未存在与已经死去。ID 仍回收，不把永不复用当合同。</summary>
    public enum EntityTombstone { NeverExisted, Dead, Alive, PendingDeath }
    /// <summary>表现原因：digest 只累计 Mutation，避免初次快照/回放被当成又一次变化。</summary>
    public enum GameplayEventCause { Mutation = 0, Initial = 1, Replay = 2 }
    ...
    public enum ResourceOperation { Add, Set, Spend }
    ...
    public enum GameplayEventType { HitConfirmed, DamageApplied, AbilityActivated, AbilityRejected, EffectApplied, EffectRejected, EffectExpired, EffectRemoved, HitMissed, DamageBlocked, HealApplied, ShieldChanged, ResourceChanged, DeathQueued, KillConfirmed, GameplayLoopAborted, AbilityCancelled }
    /// <summary>
    /// Cause 放在比较键之后：GameplayEventOrdering 仍按 sequence/type/entity，不读 Cause。
    /// digest 只累计 Mutation。
    /// </summary>
    public readonly struct GameplayEvent
    {
        ...
        public readonly EffectId EffectDefinition;
        public readonly DamageFlags Flags;
        public readonly long ParentSequence, ProvenanceId;
        public readonly int ProvenanceDepth;
        public readonly int ProducerIndex;
        public readonly long Sequence;
        public readonly int Reason;
        public readonly TagId Tag;
        public readonly int OwnerPlayerId;
        public readonly GameplayEventCause Cause;
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, long sequence, int reason = 0, int ownerPlayerId = -1)
            : this(type, source, target, default(EffectHandle), default(EffectId), sequence, 0L, reason, 0, DamageFlags.None, 0L, 0, default(TagId), ownerPlayerId) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, long sequence, int reason)
            : this(type, source, target, sequence, reason, -1) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectHandle effect, long sequence, int reason = 0, int producerIndex = 0)
            : this(type, source, target, effect, default(EffectId), sequence, 0L, reason, producerIndex, DamageFlags.None, 0L, 0, default(TagId), -1) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectHandle effect, EffectId effectDefinition, long sequence, long parentSequence = 0L, int reason = 0, int producerIndex = 0, DamageFlags flags = DamageFlags.None, long provenanceId = 0L, int provenanceDepth = 0, TagId tag = default(TagId), int ownerPlayerId = -1, GameplayEventCause cause = GameplayEventCause.Mutation)
        {
            Type = type; Source = source; Target = target; Effect = effect; EffectDefinition = effectDefinition; Flags = flags;
            ParentSequence = parentSequence; ProvenanceId = provenanceId; ProvenanceDepth = provenanceDepth; ProducerIndex = producerIndex;
            Sequence = sequence; Reason = reason; Tag = tag; OwnerPlayerId = ownerPlayerId; Cause = cause;
        }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectHandle effect, EffectId effectDefinition, long sequence, long parentSequence, int reason, int producerIndex, DamageFlags flags, long provenanceId, int provenanceDepth)
            : this(type, source, target, effect, effectDefinition, sequence, parentSequence, reason, producerIndex, flags, provenanceId, provenanceDepth, default(TagId), -1) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectHandle effect, EffectId effectDefinition, long sequence, long parentSequence, int reason, int producerIndex, DamageFlags flags, long provenanceId, int provenanceDepth, TagId tag)
            : this(type, source, target, effect, effectDefinition, sequence, parentSequence, reason, producerIndex, flags, provenanceId, provenanceDepth, tag, -1) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectHandle effect, EffectId effectDefinition, DamageFlags flags, long sequence, long parentSequence, int reason, int producerIndex, long provenanceId, int provenanceDepth)
            : this(type, source, target, effect, effectDefinition, flags, sequence, parentSequence, reason, producerIndex, provenanceId, provenanceDepth, default(TagId), -1) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectHandle effect, EffectId effectDefinition, DamageFlags flags, long sequence, long parentSequence = 0L, int reason = 0, int producerIndex = 0, long provenanceId = 0L, int provenanceDepth = 0, TagId tag = default(TagId), int ownerPlayerId = -1)
            : this(type, source, target, effect, effectDefinition, sequence, parentSequence, reason, producerIndex, flags, provenanceId, provenanceDepth, tag, ownerPlayerId) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectId effectDefinition, DamageFlags flags, long sequence, long parentSequence, int reason, int producerIndex, long provenanceId, int provenanceDepth)
            : this(type, source, target, default(EffectHandle), effectDefinition, flags, sequence, parentSequence, reason, producerIndex, provenanceId, provenanceDepth) { }
        public GameplayEvent(GameplayEventType type, EntityHandle source, EntityHandle target, EffectId effectDefinition, DamageFlags flags, long sequence, long parentSequence = 0L, int reason = 0, int producerIndex = 0, long provenanceId = 0L, int provenanceDepth = 0, int ownerPlayerId = -1)
            : this(type, source, target, default(EffectHandle), effectDefinition, sequence, parentSequence, reason, producerIndex, flags, provenanceId, provenanceDepth, default(TagId), ownerPlayerId) { }
        public GameplayEvent WithCause(GameplayEventCause cause)
        {
            return new GameplayEvent(Type, Source, Target, Effect, EffectDefinition, Sequence, ParentSequence, Reason, ProducerIndex, Flags, ProvenanceId, ProvenanceDepth, Tag, OwnerPlayerId, cause);
        }
    }
