...
namespace BattleSystemECS.Systems
{
    ...
    public class TowerAttackSystem
    {
        ...
        // 并行命中只收集共享敌人状态，屏障后按 enemy/tower 稳定顺序提交。
        private readonly List<(int enemyId, int towerId, float armorShred, int invulnFrames)>[] _onHitStateQueue
            = new List<(int, int, float, int)>[2];
        private int _onHitStateQueueIdx;
        ...
        private sealed class TowerCollectBuffer
        {
            ...
            public readonly List<(int enemyId,float damage,int playerId,int towerId,int fragCount,float fragRange)> Fragment = new List<(int,float,int,int,int,float)>(4);
            public readonly List<long> CritKeys = new List<long>(4);
            public readonly int[] SelectedTargets = new int[16];

            public void Clear()
            {
                Damage.Clear(); Dodge.Clear(); Debuff.Clear(); OnHit.Clear(); Heal.Clear(); Thorns.Clear();
                Chain.Clear(); Splash.Clear(); Bounce.Clear(); Fragment.Clear(); CritKeys.Clear();
            }
        }

        private TowerCollectBuffer[] _towerCollectBuffers = Array.Empty<TowerCollectBuffer>();

        private void EnsureTowerCollectBuffers(int towerCount)
        {
            if (_towerCollectBuffers.Length < towerCount)
            {
                var expanded = new TowerCollectBuffer[towerCount];
                Array.Copy(_towerCollectBuffers, expanded, _towerCollectBuffers.Length);
                for (int i = _towerCollectBuffers.Length; i < expanded.Length; i++)
                    expanded[i] = new TowerCollectBuffer();
                _towerCollectBuffers = expanded;
            }

            for (int i = 0; i < towerCount; i++)
                _towerCollectBuffers[i].Clear();
        }

        private void DrainTowerCollectBuffers(int towerCount)
        {
            List<(int enemyId, float damage, int playerId, int towerId)> damage = _damageQueue[_damageQueueIdx];
            ...
            List<(int enemyId, float damage, int playerId, int towerId, int fragCount, float fragRange)> fragment = _fragmentQueue[_fragmentQueueIdx];

            // 按活跃塔索引和塔内收集顺序提交，结果不依赖 Parallel.For 的调度顺序。
            for (int i = 0; i < towerCount; i++)
            {
                TowerCollectBuffer collect = _towerCollectBuffers[i];
                for (int j = 0; j < collect.Dodge.Count; j++)
                    _enemyStrafeSystem?.ApplyQueuedDodge(collect.Dodge[j]);
                damage.AddRange(collect.Damage);
                debuff.AddRange(collect.Debuff);
                onHit.AddRange(collect.OnHit);
                heal.AddRange(collect.Heal);
                thorns.AddRange(collect.Thorns);
                chain.AddRange(collect.Chain);
                splash.AddRange(collect.Splash);
                bounce.AddRange(collect.Bounce);
                fragment.AddRange(collect.Fragment);
                for (int j = 0; j < collect.CritKeys.Count; j++)
                    _critFiredThisFrame.Add(collect.CritKeys[j]);
            }
        }
        ...
        public TowerAttackSystem(ComponentStore store, IRenderer logger, global::BattleSystemECS.Content.Contracts.ICombatTuningView techTreeSystem, int mapWidth, EventBus eventBus, IBattleEventBus battleEventBus = null)
        {
            ...
            _onHitStateQueue[0] = new List<(int, int, float, int)>(256);
        ...
        public void Update(float deltaTime)
        {
            ...
            // 并行阶段每个 tower index 独占一个缓冲，禁止共享 List/HashSet/Random/lock。
            EnsureTowerCollectBuffers(activeTowerIds.Count);

            Action<int> towerBody = ti =>
            {
                TowerCollectBuffer collect = _towerCollectBuffers[ti];
                ...
                if (bestTarget != -1)
                {
                    ...
                    if (towerAccuracy < 1f && DeterministicRoll(store.CurrentFrame,towerId,bestTarget,1) >= towerAccuracy) return;
                    ...
                    if (smokeMiss > 0f && DeterministicRoll(store.CurrentFrame,towerId,bestTarget,2) < smokeMiss) return;
                    ...
                    if (enemyEvasion > 0f && DeterministicRoll(store.CurrentFrame,towerId,bestTarget,3) < enemyEvasion) return;

                    // 并行阶段只收集 dodge 事实，位置/冷却在屏障后稳定提交。
                    if (_enemyStrafeSystem != null
                        && _enemyStrafeSystem.TryQueueDodge(bestTarget, towerId >= 0 ? 1 : -1, towerId, out var dodgeFact))
                    {
                        collect.Dodge.Add(dodgeFact);
                        return;
                    }
                    ...
                    if (dmgVariance >0f)
                    baseDmg *= (float)(1.0 - dmgVariance + DeterministicRoll(store.CurrentFrame,towerId,bestTarget,4) * dmgVariance *2.0);
                    ...
                    switch (towerType)
                    {
                        case TowerType.AOE:
                            ...
                            collect.Damage.Add((bestTarget, baseDmg, store.PlayerEntityId, towerId));
                        ...
                        case TowerType.Sniper:
                            ...
                            if (store.EnemyArmor[bestTarget] >= 0)
                            {
                                ...
                                float markAmp = 0.20f;
                                if (markAmp > 0f)
                                    collect.Debuff.Add((bestTarget, towerId));
                            }
                            break;
                        ...
                        case TowerType.Tesla:
                            ...
                            collect.Chain.Add((0, bestTarget, baseDmg, store.PlayerEntityId, towerId));
                            break;
                        ...
                        case TowerType.Leech:
                            ...
                            if (healAmount > 0f)
                                collect.Heal.Add((store.PlayerEntityId, healAmount));
                            break;
                        ...
                        default:
                            ...
                            if (store.TowerArmorPierceRatio[towerId] > 0f)
                            {
                                ...
                                if (pierceBonus > 0f)
                                    collect.Damage.Add((bestTarget, pierceBonus, store.PlayerEntityId, towerId));
                            }
                            ...
                            {
                                collect.OnHit.Add((bestTarget,towerId,armorShredBonus,0));
                            }
                            // Special ability: splash damage (AOE)
                            if (store.TowerSplashRadius[towerId] > 0f)
                            {
                                // Collect splash targets for later processing
                                collect.Splash.Add((bestTarget, baseDmg * 0.5f, store.PlayerEntityId, towerId));
                            }
                            ...
                            if (critRate > 0f && DeterministicRoll(store.CurrentFrame,towerId,bestTarget,5) < critRate)
                            {
                                ...
                                if (critBonus > 0f)
                                {
                                    ...
                                    // 并行段只写当前塔独占缓冲；屏障后再稳定合并到暴击集合。
                                    long critKey = ((long)bestTarget << 32) | (uint)towerId;
                                    collect.CritKeys.Add(critKey);
                                    collect.Damage.Add((bestTarget, critBonus, store.PlayerEntityId, towerId));
                                }
                            }
                            ...
                            if (projCount > 1)
                            {
                                ...
                                for (int sc = 0; sc < pelletsToFire; sc++)
                                {
                                    ...
                                    for (int k = 0; k < localCount; k++)
                                    {
                                        ...
                                        {
                                            if (collect.SelectedTargets[s] == eid) { selected = true; break; }
                                        }
                                    ...
                                    if (found >= 0)
                                    {
                                        collect.SelectedTargets[selectedCount++] = found;
                                        pelletTarget = found;
                                    }
                                    float pelletDmg = baseDmg * pelletMult;
                                    if (pelletDmg > 0f)
                                        collect.Damage.Add((pelletTarget, pelletDmg, store.PlayerEntityId, towerId));
                                }
                            }
                            ...
                            if (fragCount > 0 && projectileSystem != null)
                            {
                                ...
                                collect.Fragment.Add((bestTarget, baseDmg * fragDmgMult, store.PlayerEntityId, towerId, fragCount, fragRange));
                            }
                            ...
                            if (store.TowerBouncesRemaining[towerId] > 0 && store.TowerBounceHitsRemaining[towerId] == 0)
                            {
                                ...
                                collect.Bounce.Add((0, bestTarget, baseDmg, store.PlayerEntityId, towerId));
                            }
                            ...
                            if (multiStrikeCount > 0)
                            {
                                ...
                                for (int sc = 0; sc < extrasToHit; sc++)
                                {
                                    ...
                                    if (multiDmg > 0f)
                                        collect.Damage.Add((found, multiDmg, store.PlayerEntityId, towerId));
                                }
                            }
                    ...
                    if (invulnConfig > 0)
                    {
                        collect.OnHit.Add((bestTarget,towerId,0f,invulnConfig));
                    }
                        ...
                        && store.TowerChainDmgRatio[towerId] > 0f && store.TowerActive[chainPartnerId])
                    {
                        ...
                        collect.Damage.Add((bestTarget, chainDmg, store.PlayerEntityId, chainPartnerId));
                    }
                }
                else
                {
                    ...
                    if (dstrCount > 0)
                    {
                        ...
                        if (bestDstr != -1)
                        {
                            ...
                            // 负 enemyId 表示可破坏物，屏障后与普通伤害一并稳定提交。
                            collect.Damage.Add((-(bestDstr + 1), dstrDmg, store.PlayerEntityId, towerId));
                        }
                    }
                }
            };
            ...
            DrainTowerCollectBuffers(activeTowerIds.Count);
            ...
            foreach (var (enemyId, damage, playerId, towerId) in _damageQueue[readIdx])
            {
                        ...
                        && store.ObstacleHealth[obstacleId] > 0f)
                    {
                        ...
                        if (store.ObstacleHealth[obstacleId] <= 0f)
                        {
                            ...
                            if (effect == 1)
                            {
                                ...
                                {
                                    store.ApplyPlayerResourceAuthority(store.PlayerEntityId, store.PlayerEntityId, new Core.GAS.AttributeKey(4), effectValue);
                                }
                            }
                            else if (effect == 2)
                            {
                                ...
                                for (int ei = 0; ei < activeEnemies.Count; ei++)
                                {
                                    ...
                                    if (explosionDmg > 0f)
                                    {
                                        ...
                                        store.ApplyDamageAuthority(obstacleId, eid, explosionDmg, 0, stage: Core.GAS.DamageAmountStage.Raw);
                                    }
                                }
                            }
                ...
                store.ApplyDamageAuthority(towerId, enemyId, finalDmg, playerId, stage: Core.GAS.DamageAmountStage.PostMitigation);
                if (finalDmg > 0f)
                {
                    ...
                    if (enemyMaxMana > 0f)
                    {
                        ...
                        if (enemyCurMana > 0f)
                        {
                            ...
                            if (drain > 0f)
                            {
                                ...
                                store.ApplyEnemyResourceAuthority(enemyId, enemyId, new Core.GAS.AttributeKey(7), -drain);
                                ...
                                store.ApplyPlayerResourceAuthority(playerId, playerId, new Core.GAS.AttributeKey(7), drain);
                            }
                        }
                    }
                }
                ...
                if (store.EnemyHealth[enemyId] <= 0f)
                {
                    ...
                    if (okType == 1 && okRatio > 0f && okRadius > 0f)
                    {
                        ...
                        if (excess > 0f)
                        {
                            ...
                            _splashDamageQueue[_splashDamageQueueIdx].Add((enemyId, splashDmg, playerId, towerId));
                        }
                    }
            ...
            foreach (var (enemyId, towerId) in _debuffQueue[debuffReadIdx])
            {
                ...
                switch (towerType)
                {
                    case TowerType.Firewall:
                        ...
                        // Also roll stun
                        if (stunChance > 0f && DeterministicRoll(store.CurrentFrame,towerId,enemyId,6) < stunChance)
                        {
                    ...
                    default:
                        ...
                        if (stunChance > 0f && DeterministicRoll(store.CurrentFrame,towerId,enemyId,7) < stunChance)
                        {
                            int stunTurns = Math.Max(1, (int)Math.Ceiling(1f * (1f - _enemyStunResistance)));
                            store.ApplyEnemyStun(enemyId, stunTurns);
                        }
                        ...
                        if (disarmChance > 0f && disarmDuration > 0f && DeterministicRoll(store.CurrentFrame,towerId,enemyId,8) < disarmChance)
                        {
        ...
        private void ResolveVanguardDamageTransfer(int protectedEnemyId, float damage, float transferRatio)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                long transferProvenance = store.AllocateGameplaySequence(protectedEnemyId);
                if (!store.ApplyDamageAuthority(protectedEnemyId, vanguardId, transferredDamage, 0, flags: Core.GAS.DamageFlags.Transfer, parentSequence: transferProvenance, stage: Core.GAS.DamageAmountStage.Raw, provenanceId: transferProvenance, provenanceDepth: 1))
                {
        ...
        private void ApplyTetherDamageShare(int enemyId, float finalDmg, int playerId)
        {
            ...
            if (store.EnemyIsInvulnerable[partnerId]) return;
                ...
                store.ApplyDamageAuthority(enemyId, partnerId, finalShared, playerId, stage: Core.GAS.DamageAmountStage.PostMitigation);
            ...
            if (store.IsEnemyPendingDeath(partnerId))
            {
        ...
        private void ResolveTeslaChainLightning()
        {
            ...
            foreach (var (chainId, enemyId, damage, playerId, towerId) in _chainDamageQueue[readIdx])
            {
                ...
                else
                {
                    ...
                    if (store.EnemyIsInvulnerable[enemyId]) continue;
                    store.ApplyDamageAuthority(towerId, enemyId, damage, playerId, stage: Core.GAS.DamageAmountStage.PostMitigation);
                }
            }
        }
                ...
                if (splashRadius > 0 && splashRadius <= 100)
                {
                    ...
                    for (int ci = 0; ci < splashCount; ci++)
                    {
                        ...
                        if (falloffInnerRatio < 1.0f || falloffOuterMult < 1.0f)
                        {
                            ...
                            {
                                store.ApplyDamageAuthority(towerId, enemyId, effectiveSplash * falloffOuterMult, playerId, stage: Core.GAS.DamageAmountStage.PostMitigation);
                            }
                            else
                            {
                                store.ApplyDamageAuthority(towerId, enemyId, effectiveSplash, playerId, stage: Core.GAS.DamageAmountStage.PostMitigation);
                            }
                        }
                        else
                        {
        ...
        private void ResolveBounceDamage()
        {
            ...
            foreach (var (bounceLevel, enemyId, damage, playerId, tid) in _bounceDamageQueue[readIdx])
            {
                ...
                if (bestBounce != -1)
                {
                    ...
                    _bounceDamageQueue[writeIdx].Add((bounceLevel + 1, bestBounce, nextDmg, playerId, tid));
                    // Apply bounce damage through _damageQueue (deferred, same pattern as chain/splash)
                    _damageQueue[_damageQueueIdx].Add((bestBounce, nextDmg, playerId, tid));
                }
            }
        }
        ...
        private void ResolveOnHitState()
        {
            int readIdx=_onHitStateQueueIdx;
            int writeIdx=1-readIdx;
            _onHitStateQueueIdx=writeIdx;
            _onHitStateQueue[writeIdx].Clear();
            List<(int enemyId,int towerId,float armorShred,int invulnFrames)> events=_onHitStateQueue[readIdx];
            events.Sort(CompareOnHitState);
            for(int i=0;i<events.Count;i++)
            {
                var evt=events[i];
                if(!store.EnemyActive[evt.enemyId])continue;
                if(evt.armorShred>0f)
                {
                    store.EnemyArmorShredStacks[evt.enemyId]+=evt.armorShred;
                    store.EnemyArmorShredDuration[evt.enemyId]=Math.Max(store.EnemyArmorShredDuration[evt.enemyId],5f);
                }
                if(evt.invulnFrames>store.EnemyInvulnFramesLeft[evt.enemyId])
                    store.EnemyInvulnFramesLeft[evt.enemyId]=evt.invulnFrames;
            }
            events.Clear();
        }

        private static int CompareOnHitState((int enemyId,int towerId,float armorShred,int invulnFrames) left,
            (int enemyId,int towerId,float armorShred,int invulnFrames) right)
        {
            int enemy=left.enemyId.CompareTo(right.enemyId);
            return enemy!=0?enemy:left.towerId.CompareTo(right.towerId);
        }

        private static double DeterministicRoll(int frame,int towerId,int enemyId,int salt)
        {
            uint value=(uint)frame*0x9E3779B9u^(uint)towerId*0x85EBCA6Bu^(uint)enemyId*0xC2B2AE35u^(uint)salt*0x27D4EB2Fu;
            value^=value>>16;
            value*=0x7FEB352Du;
            value^=value>>15;
            value*=0x846CA68Bu;
