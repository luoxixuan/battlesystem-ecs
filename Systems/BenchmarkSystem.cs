...
namespace BattleSystemECS.Systems
{
    ...
    public class BenchmarkSystem
    {
        ...
        public void RunBenchmark(int scenario)
        {
            ...
            // Place towers â€?use AddTower so ActiveTowerIds is populated (matching real game flow)
            int t1 = store.CreateEntity();
            store.AddTower(t1, "å¼“ç®­å¡?, 15f, 3, 1f, 1, 50f);
            store.PositionX[t1] = 3f; store.PositionY[t1] = 15f;

            int t2 = store.CreateEntity();
            store.AddTower(t2, "é­”æ³•å¡?, 25f, 5, 1f, 1, 100f);
            ...
            // index: (int)EnemyActionType â†?direction (-1=forward, 0=stand, 1=retreat)
