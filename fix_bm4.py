...
with codecs.open('Systems/BenchmarkSystem.cs', 'r', 'utf-8') as f:
    c = f.read()

tail = '            Console.WriteLine($"[MICRO]   Steps 1-6 sum:               {t6/ticksPerMs,7:F2} ms");\n        }\n    }\n}'

new_text = '''            Console.WriteLine($"[MICRO]   Steps 1-6 sum:               {t6/ticksPerMs,7:F2} ms");
        }
...
if c.endswith(tail):
    c = c[:-len(tail)] + new_text
    with codecs.open('Systems/BenchmarkSystem.cs', 'w', 'utf-8') as f:
        f.write(c)
    print('done')
else:
