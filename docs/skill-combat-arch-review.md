...
| + | **`ElementalReactionSystem` 接线**(⚠️ 有战斗数值变化) | ✅ **已接** | registry 字段 + 构造 + `SkillBuffGroup` 槽位/调用。8 个回归测试,其中 3 个走真实 `FrameScheduler` WavePhase tick —— 拔掉接线即变红(这是唯一能捕获"系统没接上"而非"系统内部逻辑对"的证据形态,避免重演 `GetDamageMultiplier` 那类假绿) | 中(行为变更) |
